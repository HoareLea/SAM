...
namespace SAM.Analytical
{
    public static partial class Modify
    {
        ...
        //private static void Join_Tmp(this List<Tuple<Panel, double, double, Segment2D>> tuples_All, bool extend, double distance, double tolerance = Core.Tolerance.Distance)
        //{

        //    List<Tuple<Panel, BoundingBox2D, Segment2D>> tuples_Extended = new List<Tuple<Panel, BoundingBox2D, Segment2D>>();
        //    foreach (Tuple<Panel, double, double, Segment2D> tuple in tuples_All)
        //    {
        //        if(tuple.Item4 == null || tuple.Item4.GetLength() < tolerance)
        //        {
        //            continue;
        //        }

        //        Segment2D segment2D = Geometry.Planar.Query.Extend(tuple.Item4, distance, true, true);

        //        tuples_Extended.Add(new Tuple<Panel, BoundingBox2D, Segment2D>(tuple.Item1, segment2D.GetBoundingBox(), segment2D));
        //    }

        //    bool updated = false;
        //    for(int i =0; i < tuples_Extended.Count; i++)
        //    {
        //        Tuple<Panel, BoundingBox2D, Segment2D> tuple = tuples_Extended[i];

        //        List<Tuple<Panel, Segment2D, Point2D>> tuples = new List<Tuple<Panel, Segment2D, Point2D>>();

        //        foreach (Tuple<Panel, BoundingBox2D, Segment2D> tuple_Extended in tuples_Extended.FindAll(x => x != tuple && x.Item2.InRange(tuple.Item2, tolerance)))
        //        {
        //            ISAMGeometry2D geometry2D = tuple_Extended.Item3.Intersection<ISAMGeometry2D>(tuple.Item3, tolerance);
        //            if (geometry2D == null)
        //            {
        //                continue;
        //            }
        ...
        //            Tuple<Panel, double, double, Segment2D> tuple_1 = tuples_All.Find(x => x.Item1 == tuple.Item1);
        //            Point2D point2D_Closest_1 = tuple_1.Item4.GetPoints().Closest(point2D);
        //            List<Tuple<Panel, double, double, Segment2D>> tuples_1_Connected = tuples_All.FindAll(x => x.Item4.On(point2D_Closest_1, tolerance));


        //            Tuple<Panel, double, double, Segment2D> tuple_2 = tuples_All.Find(x => x.Item1 == tuple_Extended.Item1);
        //            Point2D point2D_Closest_2 = tuple_2.Item4.GetPoints().Closest(point2D);
        //            List<Tuple<Panel, double, double, Segment2D>> tuples_2_Connected = tuples_All.FindAll(x => x.Item4.On(point2D_Closest_2, tolerance));

        //            if(tuples_1_Connected.Count != 1 && tuples_2_Connected.Count != 1)
        //            {
        //                continue;
        //            }
