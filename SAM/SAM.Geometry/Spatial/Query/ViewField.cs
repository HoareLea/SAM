...
namespace SAM.Geometry.Spatial
{
    public static partial class Query
    {
        public static void ViewField<T>(this IEnumerable<T> face3DObjects, Vector3D viewDirection, out List<LinkedFace3D> linkedFace3Ds_Hidden, out List<LinkedFace3D> linkedFace3Ds_Visible, bool hidden = true, bool visible = true, double tolerance_Area = Tolerance.MacroDistance, double tolerance_Snap = Tolerance.MacroDistance, double tolerance_Distance = Tolerance.Distance) where T: IFace3DObject
        {
            ...
            List<Tuple<LinkedFace3D, List<Planar.Polygon2D>>> tuples_Hidden = Enumerable.Repeat<Tuple<LinkedFace3D, List<Planar.Polygon2D>>>(null, tuples.Count).ToList();
            List<Tuple<LinkedFace3D, List<Planar.Polygon2D>>> tuples_Visible = Enumerable.Repeat<Tuple<LinkedFace3D, List<Planar.Polygon2D>>>(null, tuples.Count).ToList();
            ...
            //for (int i =0; i < tuples.Count; i++)
            {
                ...
                foreach (Planar.Polygon2D polygon2D in polygon2Ds)
                {
                    ...
                    List<Tuple<LinkedFace3D, List<Planar.Polygon2D>>> tuples_Result = null;
                    ...
                    {
                        tuples_Result[i] = new Tuple<LinkedFace3D, List<Planar.Polygon2D>>(tuple.Item1, new List<Planar.Polygon2D>());
                    }
